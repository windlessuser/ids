...
class Diner_Model extends CI_Model{
    ...
    function get_site($host,$rank){
        $this->db->trans_start();
        $this->db->select("Site.id,name,url,description,images,Dining_And_Nightlife.rank,map,TIME_FORMAT( open, '%h:%i %p' ) as open, TIME_FORMAT( close, '%h:%i %p' ) as close,phone,email,url,street,town,district,parish,full",FALSE)
                ->from('Site')
	...
	/*if(strcmp($zone,'Kingston') == 0){
        ...
        $this->db->order_by('Dining_And_Nightlife.rank','asc');
        $site = $this->db->get();
        $this->db->trans_complete();
        if($site->num_rows > 0)
            return $site;
    ...
    function get_top($start,$stop){
        ...
        $this->db->select("Site.id,name,url,description,images,Dining_And_Nightlife.rank,map,TIME_FORMAT( open, '%h:%i %p' ) as open, TIME_FORMAT( close, '%h:%i %p' ) as close,phone,email,url,street,town,district,parish,full",FALSE)
                ...
                ->join('Address','Address.id = Site.id','left')
                ->join('Dining_And_Nightlife','Dining_And_Nightlife.id = Site.id','left')
                ->where('Dining_And_Nightlife.rank >=',$start)
                ->where('Dining_And_Nightlife.rank <=',$stop)
                ->where('Dining_And_Nightlife.id','Site.id',FALSE);
		$zone = $this->session->userdata('zone');
	/*if(strcmp($zone,'Kingston') == 0){
					$this->load->library('Subquery');
    ...
    function get_site_table(){
             $this->db->trans_start();
             $this->db->select('Site.id,name,Dining_And_Nightlife.rank,FLOOR(count / 2) as count',FALSE)
                     ->from('Site')
                     ->join('Address', 'Address.id = Site.id', 'left')
                     ->join('Dining_And_Nightlife', 'Dining_And_Nightlife.id = Address.id', 'left');
		     $zone = $this->session->userdata('zone');
	/*if(strcmp($zone,'Kingston') == 0){
					$this->load->library('Subquery');
					$sub = $this->subquery->start_subquery('where_in');
        ...
        $this->db->where('Dining_And_Nightlife.id ','Site.id',FALSE)
                     ->order_by('Dining_And_Nightlife.rank','asc');
             $sites = $this->db->get();
             $this->db->trans_complete();
               if($sites->num_rows > 0)
                   return $sites;
